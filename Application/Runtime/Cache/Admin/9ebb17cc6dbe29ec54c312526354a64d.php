...
<body>
 ...
 $pid = $_SESSION['pid']; $cid = $_SESSION['cid']; ?>
	...
	<?php case "select": ?><div class="am-form-group">
			<label  class="am-u-sm-2 am-form-label"><?php echo ($field["title"]); ?>			<?php echo ($field['value']); ?><span class="tpl-form-line-small-title"><?php echo ($field["subtitle"]); ?></span></label>
            <div class="am-u-sm-4">
                <select data-am-selected="{searchBox: 1,maxHeight: 200}" name="<?php echo ($field["name"]); ?>">
				  	 <?php if(is_array($field["other"])): $i = 0; $__LIST__ = $field["other"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($i % 2 );++$i; $selected = $field['opt']['value']==$key ? 'selected="selected" ' : ''; ?>
	...
	<?php case "noselect": ?><div class="am-form-group">
			...
			<div class="am-u-sm-4">
				<select data-am-selected="{searchBox: 1,maxHeight: 200}" name="<?php echo ($field["name"]); ?>" onchange="selectDataList(this)" id="noselect">
					<?php if(is_array($field["opt"])): $i = 0; $__LIST__ = $field["opt"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($i % 2 );++$i; $selected = $field['opt']['value']==$key ? 'selected="selected" ' : ''; ?>
						<option value="<?php echo ($key); ?>"  <?php echo ($selected); ?> ><?php echo (htmlspecialchars($option)); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
			<div class="am-u-sm-6"></div>
